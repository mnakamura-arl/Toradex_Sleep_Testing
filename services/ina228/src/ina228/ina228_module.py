# From https://github.com/adafruit/Adafruit_CircuitPython_INA228
...
from utilities import i2c

# Get the logger as configured in __main__.py
logger = structlog.get_logger()

# Common register addresses for INA288 family
_CONFIG = 0x00
_ADCCFG = 0x01
_SHUNTCAL = 0x02
_VSHUNT = 0x04
...
# Constants
_INA288_DEFAULT_ADDR = 0x40
_TEXAS_INSTRUMENTS_ID = 0x5449
_INA228_DEVICE_ID = 0x228


class Mode:
    """Operating mode constants for INA288"""
    ...
    TRIG_TEMP = 0x04
    TRIG_TEMP_BUS = 0x05
    TRIG_TEMP_SHUNT = 0x06
    TRIG_TEMP_BUS_SHUNT = 0x07
    CONT_BUS = 0x09
    CONT_SHUNT = 0x0A
    CONT_BUS_SHUNT = 0x0B
    CONT_TEMP = 0x0C
    CONT_TEMP_BUS = 0x0D
    CONT_TEMP_SHUNT = 0x0E
    CONT_TEMP_BUS_SHUNT = 0x0F
...
class ConversionTime:
    """Conversion time constants for INA288"""
...
class AveragingCount:
    """Averaging count constants for INA288"""
...
class AlertType:
    """Alert type constants for INA288"""

    NONE = 0x0
    CONVERSION_READY = 0x1
    OVERPOWER = 0x2
    UNDERVOLTAGE = 0x4
    OVERVOLTAGE = 0x8
    UNDERCURRENT = 0x10
...
class INA2XX:
    ...
    :param SMBus i2c_bus: The I2C bus the INA2XX is connected to.
    ...
    _reset = i2c.RWBit(_CONFIG, 15, register_width=2, lsb_first=False)
    _adc_range = i2c.RWBit(_CONFIG, 4, register_width=2, lsb_first=False)

    # ADC Configuration register bits
    _mode = i2c.RWBits(4, _ADCCFG, 12, register_width=2, lsb_first=False)
    _vbus_conv_time = i2c.RWBits(3, _ADCCFG, 9, register_width=2, lsb_first=False)
    _vshunt_conv_time = i2c.RWBits(3, _ADCCFG, 6, register_width=2, lsb_first=False)
    _temp_conv_time = i2c.RWBits(3, _ADCCFG, 3, register_width=2, lsb_first=False)
    _avg_count = i2c.RWBits(3, _ADCCFG, 0, register_width=2, lsb_first=False)

    # Diagnostic/Alert register bits
    _alert_latch = i2c.RWBit(_DIAGALRT, 15, register_width=2, lsb_first=False)
    _alert_conv = i2c.RWBit(_DIAGALRT, 14, register_width=2, lsb_first=False)
    _alert_polarity = i2c.RWBit(_DIAGALRT, 12, register_width=2, lsb_first=False)

    # Measurement registers
    _raw_dietemp = i2c.ROUnaryStruct(_DIETEMP, ">h")
    _raw_vbus = i2c.ROUnaryStruct(_VBUS, ">H")

    # Calibration register
    _shunt_cal = i2c.UnaryStruct(_SHUNTCAL, ">H")

    # ID registers
    _manufacturer_id = i2c.ROUnaryStruct(_MFG_UID, ">H")
    _device_id = i2c.ROUnaryStruct(_DVC_UID, ">H")

    def __init__(
        self, i2c_bus: SMBus, address: int = _INA288_DEFAULT_ADDR, skip_reset: bool = False
    ) -> None:
        self.i2c_device = i2c.I2CDevice(i2c_bus, address)
...
class INA228(INA2XX):
    """Provides necessary code for interfacing with the INA288 via I2C.

    Attributes:
        instance (int): ID of the INA288.
        port (str | int): Path to or number of the I2C bus of the instance.
        address (int): I2C address of the INA288.
        description (str): Measurement parameter(s) of the INA288.
        location (str): Physical location of the INA288.
        status (int): Status of the INA288.
        enabled_channels (tuple[int, int, int]): Channels enabled on the INA288.
    """

    # Additional registers for INA228
    _reset_accumulators_bit = i2c.RWBit(_CONFIG, 14, register_width=2, lsb_first=False)
    _alert_type = i2c.RWBits(6, _DIAGALRT, 8, register_width=2, lsb_first=False)
    _alert_flags = i2c.ROUnaryStruct(_DIAGALRT, ">H")
    _diag_alert = i2c.UnaryStruct(_DIAGALRT, ">H")  # writable view for flag clearing

    _shunt_tempco = i2c.UnaryStruct(_SHUNT_TEMPCO, ">H")
    _sovl = i2c.UnaryStruct(_SOVL, ">H")
    _suvl = i2c.UnaryStruct(_SUVL, ">H")
    _bovl = i2c.UnaryStruct(_BOVL, ">H")
    _buvl = i2c.UnaryStruct(_BUVL, ">H")
    _temp_limit = i2c.UnaryStruct(_TEMP_LIMIT, ">H")
    _pwr_limit = i2c.UnaryStruct(_PWR_LIMIT, ">H")

    _raw_vshunt = i2c.ROUnaryStruct(_VSHUNT, ">h")
    _raw_current = i2c.ROUnaryStruct(_CURRENT, ">h")
    _raw_power = i2c.ROUnaryStruct(_POWER, ">H")

    _conversion_ready = i2c.ROBit(_DIAGALRT, 1, register_width=2, lsb_first=False)
    ...
    def __init__(
            ...
            description: str,
            location: str,
            enable: tuple[int, int, int] = (0, 1, 2),
            ) -> None:
        """
        Initialize an instance with basic attributes.

        Args:
            device_id (int): ID of the instance.
            port (str | int): Path to or number of the I2C bus of the instance.
            address (int): I2C address of the instance.
            description (str): Measurement parameter(s) of the instance.
            location (str): Physical location of the instance.
            enable (tuple[int, int, int], optional): channels to initialize at start (default (0, 1, 2))
        """
        ...
        self.status: int = 0
        self.enabled_channels: tuple[int, int, int] = enable


    def __enter__(self):
        """
        Define context manager setup behavior with enhanced safety checks.
        """
        ...
        if isinstance(self._port, str) and self._port.startswith("/dev/i2c-"):
            bus_number = int(self._port.split("-")[-1])
            logger.info(f"Extracted bus number {bus_number} from {self._port}")
        else:
            bus_number = int(self._port)
        ...
        try:
            ...
            raise TypeError(f"Unable to open I2C bus: {e}") from e

        # Ping test
        try:
            self._bus.read_byte(self.address)
        except OSError as e:
            raise OSError(f"Device not found at address 0x{self.address:02x}: {e}") from e
        ...
        # Resets the device and applies base ADC configuration
        super().__init__(self._bus, self.address)

        # Verify this is actually an INA228 by checking the die ID
        logger.info("Verifying device identity")
        dev_id = self.device_id
        if dev_id != _INA228_DEVICE_ID:
            raise RuntimeError(f"Failed to find INA228 - check your wiring! (Got ID: 0x{dev_id:X})")

        # Set INA228 calibration defaults (after reset so SHUNT_CAL is retained)
        self.set_calibration(0.015, 10.0)

        logger.info(f"INA228 initialized successfully at address 0x{self.address:02x}")

        return self

    def _handle_interrupt(self, signum: int, frame: Optional[FrameType]) -> None:
        """Handle SIGINT/SIGTERM by exiting so the context manager closes the bus.

        Args:
            signum (int): Number of the received signal.
            frame (Optional[FrameType]): Current stack frame when the signal arrived.
        """
    ...
    def __exit__(
        ...
        """
        Define context manager teardown behavior. Close I2C bus and shut down logging.

        Args:
            exc_type (Optional[Type[BaseException]]): Type of exception that occurred, if any.
            exc_val (Optional[BaseException]): Instance of exception that occurred, if any.
            exc_tb (Optional[TracebackType]): Traceback object for call stack of exception, if any.
        """
    ...
    def clear_overflow_flags(self) -> None:
        ...
        flags = self._alert_flags
        self._diag_alert = flags & ~((1 << 11) | (1 << 10) | (1 << 9))
